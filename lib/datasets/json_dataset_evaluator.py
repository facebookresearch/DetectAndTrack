...
import numpy as np
import json
# TODO(rbg): investigate
# json.encoder.FLOAT_REPR = lambda o: format(o, '.3f')  # Reduce json file size
...
def evaluate_segmentations(
        json_dataset, all_boxes, all_segms, output_dir, use_salt=True,
        cleanup=False):
...
def evaluate_keypoints(
        json_dataset, all_boxes, all_keypoints, output_dir, use_salt=True,
        cleanup=False):
    res_file = os.path.join(
    ...
    # Only do evaluation on non-test sets
    if json_dataset.name.find('test') == -1:
        _do_keypoint_eval(json_dataset, res_file, output_dir)
    # Optionally cleanup results json file
    if cleanup:
        os.remove(res_file)
