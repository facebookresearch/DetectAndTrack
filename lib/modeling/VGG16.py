...
def add_VGG16_fcn(model):
    blob_out, dim_out, spatial_scale = add_VGG16_conv5_body(model)
    if cfg.MODEL.DILATION == 1:
        model.MaxPool('conv5_3', 'pool5', kernel=2, pad=0, stride=2)
        model.Conv('pool5', 'conv6', 512, 4096, 7, pad=3, stride=1)
        model.Relu('conv6', 'conv6')
        model.DropoutIfTraining('conv6')
        model.Conv('conv6', 'conv7', 4096, 4096, 1, pad=0, stride=1)
        model.Relu('conv7', 'conv7')
        blob_out = model.DropoutIfTraining('conv7')
        return blob_out, 4096, spatial_scale / 2.
    else:
        model.Conv(
            'conv5_3', 'conv6', 512, 4096, 7, pad=3 * cfg.MODEL.DILATION,
            stride=1, dilation=cfg.MODEL.DILATION)
        model.Relu('conv6', 'conv6')
        model.DropoutIfTraining('conv6')
        model.Conv('conv6', 'conv7', 4096, 4096, 1, pad=0, stride=1)
        model.Relu('conv7', 'conv7')
        blob_out = model.DropoutIfTraining('conv7')
        return blob_out, 4096, spatial_scale * cfg.MODEL.DILATION
...
def add_VGG16_roi_fc_head(model, blob_in, dim_in, spatial_scale):
    ...
    model.Relu('fc6', 'fc6')
    model.DropoutIfTraining('fc6')
    model.FC('fc6', 'fc7', 4096, 4096)
    model.Relu('fc7', 'fc7')
    blob_out = model.DropoutIfTraining('fc7')
